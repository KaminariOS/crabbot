...
#[derive(Debug, Clone)]
pub(super) struct UiApprovalRequest {
...
pub(super) fn map_daemon_stream_events(stream_events: &[DaemonStreamEnvelope]) -> Vec<UiEvent> {
...
pub(super) fn map_rpc_stream_events(stream_events: &[DaemonRpcStreamEnvelope]) -> Vec<UiEvent> {
    let mut events = Vec::new();
    for envelope in stream_events {
        match &envelope.event {
